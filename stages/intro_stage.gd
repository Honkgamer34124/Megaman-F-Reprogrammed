...
var directory:Dictionary={
	"intro_music":preload("res://assets/music/Mega Man F (Original Soundtrack) - Intro Stage Theme(Youtube).mp3")
}
...
func _on_zone_10_area_entered(area):
	if area.is_in_group("player_camera_transition_hitbox"):
		$megaman/transition_screen_timer.start()
		StageFunctions.switch_camera(player_camera,camera_10)

func _on_zone_11_area_entered(area):
	if area.is_in_group("player_camera_transition_hitbox"):
		$megaman/transition_screen_timer.start()
		StageFunctions.switch_camera(player_camera,camera_11)

func _on_zone_12_area_entered(area):
	if area.is_in_group("player_camera_transition_hitbox"):
		$megaman/transition_screen_timer.start()
		background_music.stop()
		$bosses/boss_trigger_timer/boss_music_start.start()
		StageFunctions.switch_camera(player_camera,camera_12)
