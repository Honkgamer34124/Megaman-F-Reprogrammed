...
#	$timer_start.conn
	...
	health=26
	#health=10 #for testing purposes
	
var shoot_timer=0;var blink_time=0
func _physics_process(delta):
	GlobalScript.boss_health[1]=health
	#print($hitbox_body/CollisionShape2D.disabled,$hitcooldown_timer.time_left)
...
#		velocity.y += gravity * delta
	...
	match state:
		...
		"fall_from_ceiling":
			if not is_on_floor():
				velocity.y+=(gravity+100)*delta;animated_sprite_2d.play("falling_from_ceiling")
			elif is_on_floor():
		...
		"shoot":
			...
			if animated_sprite_2d.frame==2 or animated_sprite_2d.frame==4:
				...
				if shoot_timer==1:
					...
					#if $shoot_effect.animation!="normal":
					$shoot_effect.play("normal")
					#if health<=13:
						#$repeat_proj_timer.start()
					...
					get_parent().add_child(proj_instance)
					proj_instance.global_position=$shoot_effect.global_position
						#proj_instance.projectile='pinch'
...
#					if animated_sprite_2d.animation!="falling":#prevents the animation from playing again
					animated_sprite_2d.play("falling")
...
func _on_animated_sprite_2d_animation_finished():
	match animated_sprite_2d.animation:
		"fall_recover":
			...
			#$timer_start_attack.start(0.5)
#		"fall_recover","shoot":
#			state_random=randi_range(1,2)
		"shoot":
			$timer_jump.start()
		"falling":
			if is_on_floor():
