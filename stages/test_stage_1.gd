...
var boss_timer=0;var health_boss_update_trigger=false
...
func _process(_delta):
	...
	if GlobalScript.boss_health[1]<=0:
		...
		if timer<5:
			timer+=1
		if timer==1:$bg_test_stage1.play()
#		if $bg_test_stage1.
	else:
		$all_doors/door5.set_physics_process(false)
	
	if $bg_test_stage1.is_playing()==false:pass
...
#		$bg_test_stage1.volume_db-=0.1
	...
	if $green_devil_proto.change_animations==300:#creates tween for increasing health
		...
		tween.tween_property($megaman/player_camera/hud/boss_healthbar,"value",$green_devil_proto.max_health,2)
...
var has_entered=false;var boss_music_started=false
