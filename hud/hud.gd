...
var weapon_energy_not_all_full=false;var array
@export var array_weapons_used:Array[Sprite2D]
# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(_delta):
	if MegamanItems.weapon_number!=0 and MegamanItems.weapon_color_dictionary_bodycolor1.has(MegamanItems.weapon_number):
	...
	if isGamePaused==false:
		if Input.is_action_just_pressed("pause_game") and $pauseGameCDownTimer.time_left<=0:
			...
			if GlobalScript.health>0:
				$pause_menu.play()
	if Input.is_action_just_pressed("pause_game") and isGamePaused==true and $pauseGameCDownTimer.time_left<=0: 
	...
	show_weapon_energy_on_hud();set_volume_of_music_on_pause()
	...
	for i in array_weapons_used:
			...
			if i !=array_weapons_used[MegamanItems.weapon_number] and i!=null and array_weapons_used[MegamanItems.weapon_number]!=null:
...
#			DisplayServer.window_set_size(screen_sizes.get(2))
		...
		match select_menu_index:
			0: selection_square.global_position=$"pause_screen_setup/menu_arrangements/weapon_display/icons+text/buster".global_position
			1: selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/wp1'.global_position
			2:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/wp2'.global_position
			3:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/wp3'.global_position
			4:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/wp4'.global_position
			5:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/wp5'.global_position
			6:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/magpulse'.global_position
			7:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/wp7'.global_position
			9:selection_square.global_position=$'pause_screen_setup/menu_arrangements/weapon_display/icons+text/rush_coil'.global_position
			11:
				$pause_screen_setup/menu_arrangements/select.global_position=$pause_screen_setup/menu_arrangements/e_tank/energy_tank_icon.global_position
				if Input.is_action_just_pressed("shoot") and GlobalScript.health<GlobalScript.max_health:
					if GlobalScript.energy_tank_number>0:
						GlobalScript.energy_tank_number-=1
						disabled_selection=true
						var life_tween=create_tween()
						life_tween.tween_property($pause_screen_setup/healthbar_pausescreen,'value',GlobalScript.max_health,.5)
						life_tween.connect('finished',life_tween_ended)
						#GlobalScript.health=GlobalScript.max_health
			12:
				$pause_screen_setup/menu_arrangements/select.global_position=$pause_screen_setup/menu_arrangements/w_tank/weapon_tank_icon.global_position
				if Input.is_action_just_pressed("shoot") and weapon_energy_not_all_full:
					if GlobalScript.weapon_tank_number>0:
						...
						for i in $pause_screen_setup/menu_arrangements/weapon_display/weapon_bars.get_children():
							var weapon_tween=create_tween()
							weapon_tween.tween_property(i,'value',27,.5)
							weapon_tween.connect('finished',weapon_tween_ended)
						#GlobalScript.health=GlobalScript.max_health
			13:
				$pause_screen_setup/menu_arrangements/select.global_position=$pause_screen_setup/menu_arrangements/m_tank/mega_tank_icon.global_position
				if Input.is_action_just_pressed("shoot") and (weapon_energy_not_all_full or GlobalScript.health<GlobalScript.max_health):
...
var color_current;var darkened;var disabled_selection=false
func fade_effect():
...
func health_enemies():
	if index_enemy_health==1:
		...
		$boss_healthbar.texture_progress=preload('res://assets/miscellaneous/progress_bars/bosses/mini_yellow_devil_healthbar.png')
		$health_backup3.position.y=$health_backup2.position.y
		$boss_healthbar.set_global_position($health_backup3/miniyellowdevil.global_position-Vector2(0,4))
...
func update_values():
	...
	$pause_screen_setup/menu_arrangements/e_tank/e_tank_no.text=energy_tank_no_temporal_value.pad_zeros(2)
	var weapon_tank_no_temporal_value=str(GlobalScript.weapon_tank_number)
	$pause_screen_setup/menu_arrangements/w_tank/w_tank_no.text=weapon_tank_no_temporal_value.pad_zeros(2)
	var mega_tank_temporal_value=str(GlobalScript.mega_tank_number)
	$pause_screen_setup/menu_arrangements/m_tank/m_tank_no.text=str(mega_tank_temporal_value).pad_zeros(2)
...
func show_weapon_energy_on_hud():
	...
	if MegamanItems.weapon_energy.has(MegamanItems.weapon_number):
		$weapon_energy_display/weapon_energy.value=MegamanItems.weapon_energy[MegamanItems.weapon_number]
		$weapon_energy_display/weapon_energy_text.text=str(MegamanItems.weapon_energy[MegamanItems.weapon_number]).pad_zeros(2)
...
func update_weapon_values():
	$pause_screen_setup/menu_arrangements/weapon_display/weapon_bars/m_pulse.value=MegamanItems.weapon_energy[6]
	$pause_screen_setup/menu_arrangements/weapon_display/weapon_bars/r_coil.value=MegamanItems.weapon_energy[9]
